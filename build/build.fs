...
let productName = "Playwright.FSharp.Dsl"
let sln = __SOURCE_DIRECTORY__ </> ".." </> "Playwright.FSharp.Dsl.sln"
...
let dslSrcDir = __SOURCE_DIRECTORY__ </> ".." </> "src" </> "Playwright.FSharp.Dsl"
let dslGeneratorProject = __SOURCE_DIRECTORY__ </> ".." </> "src" </> "DslGenerator" </> "DslGenerator.fsproj"

let gitOwner = "ThisFunctionalTom"
let gitRepoName = "Playwright.FSharp.Dsl"
...
let generateDsl _ =
    dotnet.run (fun args ->
        { args with WorkingDirectory = dslSrcDir }
    ) (sprintf " --project %s" dslGeneratorProject)
    |> failOnBadExitAndPrint
...
let initTargets () =
    ...
    Target.create "GenerateDsl" generateDsl
    ...
    // Only call GeneratedDsl if Publish was in the call chain
    // Ensure GenerateDsl is called after DotnetRestore and before DotnetBuild
    "DotnetRestore" ?=>! "GenerateDsl"
    "GenerateDsl" ?=>! "DotnetBuild"
    "GenerateDsl" ==>! "PublishToNuGet"
